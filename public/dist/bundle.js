...
function doAjax(data, target) {
  ...
  jqXHR = jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
    ...
    // 子要素を削除
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.player-select').empty();
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()("".concat(target, " .player-select")).append("<ul class=\"player-list bg-gray-800 w-[120%] mt-1.5 absolute z-50\">".concat(li, "</ul>"));
    ...
    if (json.length === 1) {
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('.player-list').remove();
...
function textData(squadArray, squadNewKRSumAry, killRateAverage, squadMaxKillRate, squadMinKillRate) {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#do-copy').append(copyResult);
...
var csvReader = fileReader.onload = function () {
  ...
  // entryArray を初期化
  _entry_js__WEBPACK_IMPORTED_MODULE_1__.entryArray.splice(0);

  // 子要素を削除（エントリープレイヤー）
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#entry').empty();
...
function draw() {
  ...
  for (var i = 0; i < _entry_js__WEBPACK_IMPORTED_MODULE_1__.entryArray.length; i++) {
    if (_entry_js__WEBPACK_IMPORTED_MODULE_1__.entryArray[i] === undefined) {
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('#entry').after("\n      <div class=\"entry-error text-red-700 text-2xl mt-5 animate-fadeIn text-center\">\n        <p>No.".concat(i + 1, " kill rate is not a number.</p>\n      </div>"));
      playerNameClass[i].classList.add("error-empty");
      // 抽選中の Loading を削除
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('#draw-btn .loading').css('display', 'none');
      return;
  ...
  // 入力フォームのエラーメッセージ
  if (_entry_js__WEBPACK_IMPORTED_MODULE_1__.entryArray.length < 4) {
    //4人以上いなければエラーを返す
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#entry').after("\n    <div class=\"entry-error text-red-700 text-2xl mt-5 animate-fadeIn text-center\">\n      <p>There are only ".concat(_entry_js__WEBPACK_IMPORTED_MODULE_1__.entryArray.length, " people. We need more than 4 people to draw lots.</p>\n    </div>"));
    // 抽選中の Loading を削除
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#draw-btn .loading').css('display', 'none');
    return;
  }
  ...
  // 抽選する条件
  var SquadPlayerSum = Number(document.getElementById("squad-player").value);
  ...
  setTimeout(function () {
    var _loop = function _loop() {
      ...
      var m = 0;
      // 抽選されたスクワッドのキルレート
      var squadKillRateSum;
      var squadKillRateSumArray = [];

      // プレイヤー数
      var playerSum = _entry_js__WEBPACK_IMPORTED_MODULE_1__.entryArray.length;
      ...
      if (playerSum % SquadPlayerSum == 1 && SquadPlayerSum == 3) {
        while (k < playerSum) {
          ...
          if (k == playerSum - 4) {
            ...
            squadKillRateSum = squadArray[m + 1].reduce(function (sum, j) {
              return sum + j.kill_rate;
            }, 0);
            // キルレートの合計を配列に追加
            squadKillRateSumArray.push(squadKillRateSum);
      ...
      // 通常の処理
        ...
        for (var _i3 = 0; _i3 < playerSum; _i3 = _i3 + SquadPlayerSum, m++) {
          // シャッフルされたメンバーの配列をスクワッド毎に分割
          squadArray.push(shuffleArray.slice(_i3, _i3 + SquadPlayerSum));
          squadKillRateSum = squadArray[m].reduce(function (sum, j) {
            return sum + j.kill_rate;
          }, 0);
          // キルレートの合計を配列に追加
      ...
      if (compare()) {
        ...
        // 平均のキルレート値
        var killRateAverage = killRateSum / drawArray.length;
        ...
        squadMinKillRate = Math.min.apply(Math, squadNewKRSumAry);
        // 配列の順序[i]を取得
        var squadMaxKillRatePosition = squadNewKRSumAry.indexOf(squadMaxKillRate);
        var squadMinKillRatePosition = squadNewKRSumAry.indexOf(squadMinKillRate);
        ...
        (0,_copy_js__WEBPACK_IMPORTED_MODULE_2__.textData)(squadArray, squadNewKRSumAry, killRateAverage, squadMaxKillRate, squadMinKillRate);
        ...
        // splideのスライダーを削除
        var splideClass = document.getElementsByClassName("splide");
        ...
        if (streaming.checked) {
          // 配信用のスライダー
          var splideJs = function splideJs() {
            var sliderElm = document.createElement("section");
            jquery__WEBPACK_IMPORTED_MODULE_0___default()(sliderElm).addClass("splide");
            jquery__WEBPACK_IMPORTED_MODULE_0___default()(sliderElm).addClass("mt-10");
            var sliderElm2 = document.createElement("div");
            jquery__WEBPACK_IMPORTED_MODULE_0___default()(sliderElm2).addClass("splide__track");
            var sliderElm3 = document.createElement("div");
            jquery__WEBPACK_IMPORTED_MODULE_0___default()(sliderElm3).addClass("splide__list");
            jquery__WEBPACK_IMPORTED_MODULE_0___default()('#draw-btn').append(sliderElm);
            jquery__WEBPACK_IMPORTED_MODULE_0___default()('.splide').append(sliderElm2);
            jquery__WEBPACK_IMPORTED_MODULE_0___default()('.splide__track').append(sliderElm3);
            for (var _i7 = 0; _i7 < squadArray.length; _i7++) {
              var _createSquadSection = document.createElement("section");
              jquery__WEBPACK_IMPORTED_MODULE_0___default()(_createSquadSection).addClass("splide__slide");
              var _createSquadDiv = document.createElement("div");
              _createSquadDiv.classList.add("bg-slate-800", "mt-2", "py-10", "px-32");
              jquery__WEBPACK_IMPORTED_MODULE_0___default()('.splide__list').append(_createSquadSection);
              _createSquadSection.appendChild(_createSquadDiv);
              _createSquadSection.insertAdjacentHTML("afterbegin", "<h2 class=\"text-5xl font-anton m-5 text-left\">Group ".concat(alphabet[_i7], "</h2>"));
              for (var _j2 = 0; _j2 < squadArray[_i7].length; _j2++) {
                var _resultHtml = "\n                <div class=\"player grid grid-cols-[150px_1fr_140px] gap-3 items-center mt-5\">\n                  <figure><img src=\"".concat(squadArray[_i7][_j2].avatar, "\"></figure>\n                  <div class=\"font-barlow_condensed text-7xl leading-none break-all\">").concat(squadArray[_i7][_j2].player_name, "</div>\n                  <div class=\"font-bebas_neue text-5xl leading-none text-right\">").concat(squadArray[_i7][_j2].kill_rate.toFixed(2), "</div>\n                </div>");
                _createSquadDiv.insertAdjacentHTML("beforeend", _resultHtml);
                _createSquadSection.appendChild(_createSquadDiv);
              }
              _createSquadDiv.insertAdjacentHTML("afterbegin", "\n              <div class=\"player-header font-caveat text-5xl grid grid-cols-[1fr_140px] justify-items-center mb-10\">\n                <div class=\"text-center\">player name</div>\n                <div>kill rate</div>\n              </div>");
              // スクワッドの合計・平均キルレート
              var _squadNewKRSum = squadArray[_i7].reduce(function (sum, j) {
                return sum + j.kill_rate;
              }, 0);
              squadNewKRSumAry.push(_squadNewKRSum);
              _createSquadDiv.insertAdjacentHTML("beforeend", "<div class=\"kill-rate-average font-caveat text-3xl mt-5 text-right\">Average kill rate: ".concat((squadNewKRSumAry[_i7] / squadArray[_i7].length).toFixed(2), "</div>"));
              _createSquadDiv.insertAdjacentHTML("beforeend", "<div class=\"kill-rate-sum text-3xl font-caveat text-right\"><span>Total kill rate: ".concat(squadNewKRSumAry[_i7].toFixed(2), "</span></div>"));
            }
          };
          splideJs();
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('#draw-btn .loading').css('display', 'none');
        // Stream用のスライドを削除
        var _splideClass = document.getElementsByClassName("splide");
        while (_splideClass.length > 0) {
          _splideClass[0].parentNode.removeChild(_splideClass[0]);
        }
      }
      count++;
    };
...
function ScrollWindow(elem) {
  ...
  var streaming = document.getElementById("streaming");
  var splideC = document.querySelector('.splide');
  if (streaming.checked) {
    splideC.scrollIntoView({
      behavior: 'smooth',
      block: 'center'
    });
  } else if (element) {
...
function newPlayerCreate() {
  ...
  // プレイヤー数
  var playerSum = document.getElementsByClassName("entry-player").length;
  // プレイヤー名入力の親Div作成
  ...
  // 人数超過のエラーメッセージ
  if (playerSum >= maxPlayer) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#entry').after("<div class=\"entry-error animate-fadeIn\"><p class=\"error\">Up to ".concat(maxPlayer, " players can participate.</p></div>"));
    // エントリー追加アイコンを設置
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#entry').append('<div id="add-button" class="hover:opacity-50 hover:cursor-pointer"><img src="./images/plus-icon.png" alt="メンバー追加">');
  }
}
...
function deletePlayer(event) {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(deleteTarget).remove();
 ...
 */
jquery__WEBPACK_IMPORTED_MODULE_0___default()('#entry').on('keyup', '.input-player', function (event) {
  // setTimeoutが実行中だったらカウントダウンをストップする
  ajaxClearTimeoutId();
  var target = "#player".concat(jquery__WEBPACK_IMPORTED_MODULE_0___default()(event.currentTarget).data("id")); // 親要素の ID を設定
  var playerName = jquery__WEBPACK_IMPORTED_MODULE_0___default()("".concat(target, " .input-player")).val(); // 入力されたユーザー名
  var data = {
    'player': playerName
  };
...
function ajaxSetTimeoutId(data, target) {
  ...
  timeoutId = setTimeout(function () {
    // ajax通信を実行
    (0,_ajax_player_id_js__WEBPACK_IMPORTED_MODULE_1__.doAjax)(data, target);
  }, 1000); // 指定秒後に実行
}
...
// メンバー入力欄追加
jquery__WEBPACK_IMPORTED_MODULE_0___default()("#entry").on("click", "#add-button", _entry_js__WEBPACK_IMPORTED_MODULE_2__.newPlayerCreate);

// 任意のエントリープレイヤーを削除
jquery__WEBPACK_IMPORTED_MODULE_0___default()('#entry').on('click', '.delete', _entry_js__WEBPACK_IMPORTED_MODULE_2__.deletePlayer);

// 抽選処理
jquery__WEBPACK_IMPORTED_MODULE_0___default()('#draw-btn button').on('click', _draw_js__WEBPACK_IMPORTED_MODULE_3__.draw);
})();
